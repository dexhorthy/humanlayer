...
const outerLoop = async (req: Request, res: Response) => {
  ...
  switch (body.type) {
    case "conversation.created":
      thread = new Thread([
        { type: "conversation.created", data: body.event.user_message },
      ]);
      break;
  ...
  if (body.type === "function_call.completed" && body.event.status?.approved) {
    ...
    body.type === "function_call.completed" &&
    !body.event.status?.approved
  ) {
    // add the denial to the thread
    thread.events.push({
      type: "human_response",
      data: `user denied operation ${
        thread.lastEvent().data.intent
      } with feedback: ${body.event.status?.comment}`,
    });
