  ...
  try {
    ...
    for (const email of twoEmailsShuffled) {
      ...
      const humanClassification = humanReview.approved
        ? classification
        : (humanReview.response_option_name as
            | Classification
            | null
            | undefined);
    ...
    console.log("Results:\n");
    results.forEach(({ id, subject, classification, humanClassification }) => {
      console.log(`${id}: ${subject}\nClassification: ${classification}`);
      if (classification !== humanClassification) {
        console.log(`Human classification override: ${humanClassification}\n`);
      }
    });
  } catch (error) {
