...
export const prompt = (email: Email) => `Classify this email into one of these categories:

- spam: unsolicited commercial emails, scams, or suspicious messages
- ham: emails about ham or other pork products
- action: emails requiring specific tasks or responses
- read_review: not directly actionable, but should be read soon
- other: commercial emails that are not important like greetings, events, and marketing updates
- reference: informational emails worth keeping for future reference
- archive: legitimate emails that don't need action or future reference 
...
export type Classification = 'spam' | 'ham' | 'action' | 'read_review' | 'reference' | 'archive' | 'other';
export const classificationValues: string[] = ['spam', 'ham', 'action', 'read_review', 'reference', 'archive', 'other'];

export type ClassifiedEmail = Email & {
    classification: Classification;
    has_human_reviewed?: boolean;
...
export const classifierToolsOpenai: ChatCompletionTool[] = [
    ...
    },
    {
        ...
        function: {
            name: 'action',
            description: 'a task that is actionable',
        },
    },
    {
        type: 'function',
    ...
    },
    {
        ...
        function: {
            name: 'reference',
            description: 'a task that contains information for later reference',
        },
    },
    {
        type: 'function',
        function: {
            name: 'archive',
            description: 'a task that is not worth reading and should be sent straight to the archive',
        },
    },
];
...
export async function classifyEmail(email: Email): Promise<Classification> {
    ...
    const messages: Array<OpenAI.Chat.ChatCompletionMessageParam> = [{ role: 'user', content: prompt(email) }];
    ...
    if (!toolCalls || toolCalls.length === 0) {
        console.log(`No classification received for email ${email}, response: ${JSON.stringify(response)}`);
